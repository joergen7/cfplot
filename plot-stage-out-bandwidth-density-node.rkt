...
(provide Stage-Out-Bandwidth-Density-Node-Data
...
(define-type Stage-Out-Bandwidth-Density-Node-Data
  (Listof (Pairof String (Listof Real))))

(: stage-out-bandwidth-density-node-data (History -> Stage-Out-Bandwidth-Density-Node-Data))
(define (stage-out-bandwidth-density-node-data h)
  ...
  (define (proc-entry entry)
    ...
    (for-each proc-fi stage-out-lst))

  (for-each proc-entry h)

  (hash->list t))

(: plot-stage-out-bandwidth-density-node (Stage-Out-Bandwidth-Density-Node-Data Path-String -> Void))
(define (plot-stage-out-bandwidth-density-node bwdn-data filename)
  ...
  (plot rlst
        #:x-label "Stage-out bandwidth [GiBit/s]"
